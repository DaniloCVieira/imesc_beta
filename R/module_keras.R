...
ui_keras<-function(id){
  ...
  library(tensorflow)
  library(MLmetrics)
  source("inst/app/www/funs_keras.R")
  ns<-NS(id)
...
server_keras<-function(input, output, session, vals) {
  ...
  output$page4_main<-renderUI({
    ...
    div(
      ...
      #box(renderPrint({get_pred_rinal()}))
    )
  })
  ...
  output$page2<-renderUI({
    ...
    fluidPage(
      ...
      box(uiOutput(ns("compile_inputs"))),
      box(
  ...
  observeEvent(input$nlay,{
    ...
    output$out_hidden_layers<-renderUI({
      ...
      lis<-lapply(1:input$nlay,function(i){
        ...
        box(style="border: 1px solid;margin:5px; padding: 10px",class='map_control_style2',width=4,
            h4(paste("Layer",i)),
