...
gg_grid<-function(ps,ncol,nrow,main,padding){
  ...
  title_grob <- textGrob(main, gp=grid::gpar(cex=2),vjust=0.3)
  title_gTree <- gTree(children = grid::gList(title_grob))
  ml <-grid.arrange(grobs=ps,
...
position_km_scale<-function(pxrange,pyrange,position,padding_bar){
  padding_bar=padding_bar/10
  switch(position,
         "top"={
           pos_y =   max(pyrange)
           pos_x =  mean(pxrange)
           pos_y=pos_y*(1+padding_bar)
           list(pos_y=pos_y,
                pos_x=pos_x)
         ...
         "bottomright"={
           ...
           pos_y[1] <- pyrange[1] + padding_y
           pos_x[2] <- pxrange[2] - padding_x
...
extra_coords<-function(args_extra_shape,extra_shape){
  ...
  res<-lapply(1:nrow(args_extra_shape), function(i){
    ...
    if(args_extra_shape$layers[i]=="TRUE"){
      st_coordinates(extra_shape[[i]])[,1:2]
    } else{
      NULL
    }
  })
...
get_all_coords<-function(data,base_shape_args,layer_shape_args,args_extra_shape){
  ...
  shs_sf <- lapply(shs_sf, function(x)st_coordinates(x)[,1:2])
...
map_rst<-function(data,base_shape_args,
  ...
  if(isTRUE(base_shape_args$shape)){
    ...
    if(!is.null(base_shape)) {
      ...
      my_rst<-raster::mask( raster::crop(my_rst, lims_to_extend(lims)), base_shape)
...
lims_to_extend<-function(lims,as_extent=T){
  ...
  if(isTRUE(as_extent)){
    limits<-raster::extent(raster(to_spatial(dflim)))
  } else{
    xlimits<-lims[c(1,3)]
    ylimits<-lims[c(2,4)]
    limits<-cbind(xlimits,ylimits)
...
gg_pie<-function(p,rst,factor_chart,buffer_zize,fun,min_radius,max_radius,newcolhabs,pal,reverse_palette, fillOpacity, light,leg_title=NULL){
  ...
  x<-pie_data$x
  library(scatterpie)
  p<-p+ geom_scatterpie(mapping=aes(x=x, y=y
                                    #,r=size
  ), data=pie_data,
...
step_scale_bar<-function(data,base_shape_args,
  ...
  a[1,2]<-a[2,2]
  range_meters<-max(geodist(a,measure = "geodesic"))
...
add_custom_axis<-function(p,axis_width=0.1,axis_style="default"){
  ...
  ybreaks=info$ybreaks
  pxrange=info$pxrange
  pyrange=info$pyrange
  base_limits<-c(pxrange,pyrange)
  base_limits<-base_limits[c("xmin","ymin","xmax","ymax")]
  p<-p+guides(y= guide_axis(angle=90))
  breaks_x <- c(
    base_limits["xmin"],xbreaks, base_limits["xmax"])
  breaks_y <- c(base_limits["ymin"],
                ybreaks,base_limits["ymax"])

  axis_blocks<-axis_barblock(p,longitude_breaks=breaks_x,latitude_breaks=breaks_y,base_limits=base_limits,add_km=axis_width/10)
  longitude_breaks<-axis_blocks$longitude_breaks
  latitude_breaks<-axis_blocks$latitude_breaks
  p<-p+ggnewscale::new_scale_fill()
  p<-p + guides(fill = "none")
  p<-p+
    geom_rect(data = longitude_breaks, mapping=aes(xmin=xmin,xmax=xmax,ymin=y,ymax=yh,fill=color),color="black",linewidth=0.3,show.legend=F)+
    geom_rect(data = latitude_breaks, mapping=aes(ymin=ymin,ymax=ymax,xmin=x,xmax=xh,fill=color),color="black",linewidth=0.3,show.legend=F)+scale_fill_manual(values=c("black","white"))
  p<-p+
    geom_rect(data = longitude_breaks, mapping=aes(xmin=xmin,xmax=xmax,ymin=y2,ymax=yh2,fill=color),color="black",linewidth=0.3,show.legend=F)+
    geom_rect(data = latitude_breaks, mapping=aes(ymin=ymin,ymax=ymax,xmin=x2,xmax=xh2,fill=color),color="black",linewidth=0.5,show.legend=F)
