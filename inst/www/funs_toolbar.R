...
gg_transf<-function(data,transformed){
  ...
  ggplot(df, aes(value)) +
    geom_freqpoly()+
    ggplot2::facet_wrap(vars(variable), scales = "free")+ylab("Column sums")
}
...
format_args<-list(
  backgroundColor = "#f5f5f5",columns = " "
)
option_tips <- {
  list(
    "Rename a Datalist",
...
imesc_excel_sheets<-function(path,sheet,attr="Numeric"){
  df<-data.frame(read_excel(path, sheet,na=c("","NA"),col_names =F))
...
imesc_fread<-function(path){
  ...
  data<-data.frame(fread(path, stringsAsFactors = T,na.strings=c("","NA"),header=T))
...
any_na_impute<-function(data,attr){
  if(attr=="Factor-Attribute"){
    data<-attr(data,"factors")
  }
  if(isFALSE(anyNA(data))){
    return()
  } else{
    NULL
  }
...
get_message_transf<-function(nfun){

  men<-as.character(sapply(nfun,function(x){
    if(x=="Transf"){paste(input$transf,"transformed")}else  if(x=="Scale"){paste0("scaled",if(isTRUE(input$center)){"/centered"})}else  if(x=="Rarefy"){paste0("Rarefied")}
  }))
...
create_newname<-function(saved_data,data,bag='New Datalist',action="datalist"){
  ...
  last(make.unique(c(oldnames,newname)))
}
render_small_table<-function(dt,data,height,pageLength=NULL,style0){
 d1<- if(isTRUE(dt)){
    style0<-""
    fixed_dt(head(data),height, pageLength=pageLength,dom="tp")
  } else {
    div(
      class="half-drop-inline small_table",
      renderTable(head(data),rownames =T)
    )
  }
 div(style=style0,
      d1)
...
render_list<-function(result,limited=T){
