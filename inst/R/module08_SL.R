    ...
    observeEvent(input$rank_tree,{
      ...
      withProgress(min=1,max=ntrees,message="Running",{
        res<-sapply(1:ntrees,function(i){
          ...
          metric<-postResample(pred,obs)[model$metric]
          incProgress(1)
...
render_list<-function(result, limited=T,auto=T){
  ...
  moduleServer(id,function(input,output,session){
    ...
    output$panel_green<-renderUI({
      ...
      div(
        ...
        if(length(lib)>0)
          if(lib!="NULL")
            p(tiphelp("R package"),strong('Library:'),style="white-space: normal;",
              ...
              tipify(actionLink(ns("model_help"),icon('fas fa-question-circle')),"Access the R documentation of the model","right"),
        ...
        p(tipify(icon('tag'),"Subjects associated with the model"),strong('tags:'),style="white-space: normal;",
    ...
    observeEvent(input$run_train,ignoreInit = T,{
      vals$model_error0<-try({
        ...
        trControl=do.call(trainControl,trControl_list)
        ...
        div(style="display: flex;gap: 10px;",
            ...
            div(style="display: flex;",id=ns("model_model_name"),
                ...
                div(class="save_changes",
                    ...
                    tipify(actionButton(ns("save_model"),icon("fas fa-save",style="color: #374061ff")),"Save the model in the current Datalist X", "right")),
                div(style="padding-top: 20px; margin-left: 10px",
                    tipify(actionButton(ns("trash_model"),icon("trash")),"Click to remove the model","right"))
            )
...
div(
  ...
  div(
    ...
    observeEvent(vals$cmodel,{
      req(vals$cmodel)
      type=sort(getModelInfo(vals$cmodel,regex = F)[[1]]$type)
      model_type(type)
    })
